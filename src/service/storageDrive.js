...
// todo: https://developers.google.com/drive/v3/reference/files/update
function write(fileName,data){
  ...
  return new Promise((resolve,reject)=>{
    request.execute((jsonResp,rawResp)=>{
      ...
      console.log('upload',isUpdate,{jsonResp,rawResp}); // todo: remove log
...
function search(fileName){
    ...
    gapi.client.drive.files.list({
      ...
      console.log('search',fileName,files); // todo: remove log
    },reject);
  })
}
...
export default {
  name
  ,init
  ,get authorised() { return isAuthorised }
  ,read
  ,write
}
